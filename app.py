...
# Listas separadas para evitar conflitos de rotas na API
TICKERS_ACOES = [
...
def buscar_proventos_geral(token_brapi=None):
    ...
    # 1. Consulta Ações (Endpoint v2 /stocks/dividends)
    if TICKERS_ACOES:
        ...
        for lote in lotes_acoes:
            url = f"https://brapi.dev/api/v2/stocks/dividends?symbols={','.join(lote)}"
            try:
    ...
    # 2. Consulta FIIs (Endpoint v2 /fii/dividends)
    if TICKERS_FIIS:
        tickers_fiis_limpos = [t.replace(".SA", "") for t in TICKERS_FIIS]
        print("Varrendo FIIs...")
        for ticker in tickers_fiis_limpos:
            url = f"https://brapi.dev/api/v2/fii/dividends?symbols={ticker}"
            try:
                res = requests.get(url, headers=headers, timeout=15)
                if res.status_code == 200:
                    for item in res.json().get("dividends", []):
                        # FIIs utilizam comumente 'dataCom' ou 'approvedOn'
                        data_com_str = item.get("dataCom") or item.get("approvedOn")
                        if not data_com_str: continue
                        
                        data_com = pd.to_datetime(data_com_str).tz_localize(None).normalize()
                        if inicio_janela <= data_com <= fim_janela:
                            valor = item.get("cashDividends", 0) or item.get("value", 0)
                            proventos_encontrados.append(
...
if __name__ == "__main__":
    ...
    resultados = buscar_proventos_geral(token)
