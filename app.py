...
# Exemplo de lista com seus tickers (mantendo o sufixo .SA)
TICKERS_B3 = [
...
def buscar_dados_em_lote(tickers):
    print(f"Iniciando download em lote para {len(tickers)} ativos...")

    # yf.download busca todos os ativos em paralelo usando threads
    dados = yf.download(
        tickers=tickers,
        period="5d",  # Traz os últimos 5 dias de mercado
        group_by="ticker",  # Organiza a resposta por código do ativo
        threads=True,  # Habilita o paralelismo (Multi-threading)
        progress=False,  # Desativa a barra de progresso no log do GitHub Actions
    )

    proventos_e_cotacoes = {}

    for ticker in tickers:
        try:
            # Quando baixamos múltiplos tickers, a estrutura do DataFrame é um MultiIndex
            df_ticker = dados[ticker]

            # Verifica se o ticker retornou dados válidos
            if not df_ticker.empty and "Close" in df_ticker.columns:
                # Remove linhas onde a cotação veio vazia (NaN)
                df_valido = df_ticker.dropna(subset=["Close"])

                if not df_valido.empty:
                    ultima_cotacao = df_valido["Close"].iloc[-1]

                    proventos_e_cotacoes[ticker] = {
                        "cotacao": round(float(ultima_cotacao), 2),
                        "data": df_valido.index[-1].strftime("%Y-%m-%d"),
                    }
        except Exception as e:
            # Caso algum ativo específico falhe ou tenha sido fechado/fechado na B3
            continue

    return proventos_e_cotacoes


if __name__ == "__main__":
    resultados = buscar_dados_em_lote(TICKERS_B3)
    print(
        f"Sucesso! Processados {len(resultados)} ativos com cotações válidas."
    )
